    ...
    document.getElementById('main-checkout-btn').addEventListener('click', () => {
        ...
        const handler = PaystackPop.setup({
            ...
            callback: function(res) {
                let itemsStr = "";
                cart.forEach(item => { itemsStr += `(${item.quantity}x) ${item.name} - ${item.size}/${item.color}%0A`; });
...
function confirmAddToCart() {
    const container = document.getElementById('modal-options-container');
    const size = container.querySelector('.size-btn.active')?.dataset.size;
    const color = container.querySelector('.color-btn.active')?.dataset.color;
    const qty = parseInt(document.getElementById('modal-qty-num').innerText);
    if (!size || !color) { alert("SELECT SIZE & COLOR"); return; }
    ...
    else { cart.push({ name: currentProduct.name, price, size, color, img: currentProduct.img, quantity: qty }); }
    saveAndRefresh();
    closeModal();
    toggleCart(true);
}

function toggleControls(card) { openProductModal(card); }

function toggleCart(forceOpen = false) {
    const sidebar = document.getElementById('cart-sidebar');
    const overlay = document.querySelector('.cart-overlay');
    if(forceOpen) { sidebar.classList.add('open'); overlay.style.display = 'block'; } 
    else { sidebar.classList.toggle('open'); overlay.style.display = sidebar.classList.contains('open') ? 'block' : 'none'; }
}
...
function saveAndRefresh() { localStorage.setItem('newWaveCart', JSON.stringify(cart)); updateUI(); }
...
function updateUI() {
    ...
    cart.forEach((item, index) => {
        ...
        container.innerHTML += `<div class="cart-item"><img src="${item.img}" class="cart-thumb"><div class="item-info"><h4>${item.name}</h4><p>${item.size} / ${item.color}</p><div class="qty-ctrl"><button class="qty-btn" onclick="updateQty(${index}, -1)">-</button><span class="qty-num">${item.quantity}</span><button class="qty-btn" onclick="updateQty(${index}, 1)">+</button></div></div></div>`;
    });
